...
$(document).ready(function() {
  ...
  function add(){
		jp.go("${ctx}/portal/rlzyUser/form/add");
	}
  ...
  function edit(id){
	  ...
	  jp.go("${ctx}/portal/rlzyUser/form/edit?id=" + id);
  }
 ...
 function view(id){//没有权限时，不显示确定按钮
      if(id == undefined){
         ...
         jp.go("${ctx}/portal/rlzyUser/form/view?id=" + id);
 }
  ...
  function detailFormatter(index, row) {
	  ...
	  $.get("${ctx}/portal/rlzyUser/detail?id="+row.id, function(rlzyUser){
		...
		for (var i=0; i<data1.length; i++){
			...
			data1[i].dict.experiencetype = jp.getDictLabel(${fns:toJson(fns:getDictList('experience_type'))}, data1[i].experiencetype, "-");
