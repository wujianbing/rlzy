...
<label><input name="${name}" type="checkbox" value="${item.value}" 
<c:if test="${fn:contains(','.concat(values).concat(','), ','.concat(item.value).concat(','))}">checked</c:if> class="${cssClass}"/>${item.label} </label>
</c:forEach>
